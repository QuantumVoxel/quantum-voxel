...
public class InspectorDebugPage implements DebugPage {
    ...
    private void handleEnterKey(DebugPageContext context, String path, Comparator<InspectionNode<?>> comparator) {
    ...
    private boolean renderNodes(DebugPageContext context, String path, Comparator<InspectionNode<?>> comparator) {
        @Nullable InspectionNode<?> node = context.client().inspection.getNode(path);
        if (node == null) {
            this.inspectCurrentPath = "/";
            return true;
