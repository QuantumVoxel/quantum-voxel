...
public class EnumEntry<T extends Enum<T>> extends ConfigEntry<T> {
    ...
    public Widget createButton(Config options, int x, int y, int width) {
        CycleButton<T> cycleButton = new CycleButton<T>().formatter(enumValue -> TextObject.nullToEmpty(enumValue.name())).values(clazz.getEnumConstants()).index(this.get().ordinal()).callback((cycler) -> {
