...
public class HungerOverlay extends Overlay {
    ...
    protected void render(Renderer renderer, float deltaTime) {
        ...
        for (int emptyHeartX = 0; emptyHeartX < 10; emptyHeartX++)
            renderer.blit(iconsTex, x + emptyHeartX * 8, leftY - 9, 9, 9, 16, 10);

        int heartX;
        for (heartX = 0; heartX < Math.floor(player.getHealth() / 2); heartX++)
            renderer.blit(iconsTex, x - heartX * 8, leftY - 9, 9, 9, 25, 10);

        if ((int) player.getHealth() % 2 == 1)
            renderer.blit(iconsTex, x - heartX * 8, leftY - 9, 9, 9, 34, 10);
