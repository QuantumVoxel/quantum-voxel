...
public class ClientTcpConnection extends Connection<ClientPacketHandler, ServerPacketHandler> {
    ...
    private ClientTcpConnection(Socket socket, UltracraftClient client) {
        super(socket, client);
        this.client = client;
    }
    ...
    public static Result<ClientTcpConnection> connectToServer(String address, int port) {
        try {
            return Result.ok(new ClientTcpConnection(new Socket(address, port), UltracraftClient.get()));
        } catch (IOException e) {
