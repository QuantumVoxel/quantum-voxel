...
public class SocketConnection extends SimpleChannelInboundHandler<PacketSequence<?>> implements Connection {
    ...
    public static final Supplier<NioEventLoopGroup> LOCAL_WORKER_GROUP = Suppliers.memoize(SocketConnection::createLocalWorkerGroup);
    public static final Supplier<EpollEventLoopGroup> NETWORK_EPOLL_WORKER_GROUP = Suppliers.memoize(SocketConnection::createNetworkEpollWorkerGroup);

    public static final Supplier<NioEventLoopGroup> NETWORK_WORKER_GROUP = Suppliers.memoize(SocketConnection::createNetworkWorkerGroup);
    ...
    public SocketConnection(PacketDestination direction) {
    ...
    private void handleInternalError(Throwable cause, boolean handlingFault, Channel channel) {
        ...
        if (handlingFault) {
            Connection.LOGGER.error("Double fault detected, force closing SocketConnection.");
            try {
                channel.close().addListener(SocketConnection::closeFail);
            } catch (Exception e) {
    ...
    public void disconnect(@NotNull String message) {
        ...
        // Send the appropriate disconnect packet based on the SocketConnection direction
        ...
        // Set the SocketConnection to read-only mode
        this.setReadOnly();
    }
    ...
    private void _send(@NotNull Packet<?> packet, @Nullable PacketResult stateListener, boolean flush) throws ClosedChannelException {
        ...
        // Throw exception if the channel is not open
    ...
    private void _actuallySend(Packet<?> packet, @Nullable PacketResult stateListener, boolean flush) {
        ...
        try {
            ...
            // Send the packet with or without flushing based on 'flush' parameter
    ...
    /**
     * Checks if the SocketConnection is currently connected to a server or the client.
     *
     * @return {@code true} if the SocketConnection is currently connected, {@code false} otherwise
     */
    @Override
    ...
    @Override
    protected void channelRead0(ChannelHandlerContext ctx, PacketSequence<?> msg) {
        ...
        if (this.channel != null && this.channel.isOpen()) {
            ...
            try {
                ...
                this.readGeneric(msg.sequence(), msg.packet(), handler);
    ...
    private <T extends PacketHandler> void readGeneric(long sequenceId, Packet<T> msg, @NotNull PacketHandler handler) {
        ...
        if (handler.isAsync()) {
            CompletableFuture.runAsync(() -> {
                try {
                    ...
                    if (sequenceId != -1)
                        this.send(handler.reply(sequenceId), true);
                } catch (Exception e) {
    ...
    /**
     * Gets the direction of the SocketConnection.
     *
     * @return the direction of the SocketConnection
     */
    @Override
    ...
    /**
     * Sets the initial attributes for the SocketConnection.
     *
     * @param channel the Netty channel to set the attributes on.
     */
    public static void setInitAttributes(Channel channel) {
        channel.attr(SocketConnection.DATA_TO_CLIENT_KEY).set(PacketStages.LOGIN.getClientBoundData());
        channel.attr(SocketConnection.DATA_TO_SERVER_KEY).set(PacketStages.LOGIN.getServerData());

        channel.config().setRecvByteBufAllocator(new AdaptiveRecvByteBufAllocator(64, 8192, 1024 * 1024 * 2));
    }

    /**
     * Moves to the in-game SocketConnection stage.
     */
    @Override
    public void moveToInGame() {
        if (this.channel != null && this.channel.isOpen()) {
            this.channel.attr(SocketConnection.DATA_TO_CLIENT_KEY).set(PacketStages.IN_GAME.getClientBoundData());
            this.channel.attr(SocketConnection.DATA_TO_SERVER_KEY).set(PacketStages.IN_GAME.getServerData());
        }
    }

    /**
     * Sets up the pipeline to handle packets.
     * Adds the packet encoding and decoding, and optionally ZLib compression (when this is not a memory SocketConnection).
     *
    ...
    public void setup(ChannelPipeline pipeline) {
        try {
            ...
            var ourData = SocketConnection.getDataKey(this.direction);
            var theirData = SocketConnection.getDataKey(oppositeDirection);
...
//            pipeline.addLast("partial_decoder", new PartialPacketDecoder())
//                    .addLast("partial_encoder", new PartialPacketEncoder());
//            pipeline.addLast("splitter", new PacketSplitter())
//                    .addLast("combiner", new PacketCombiner());
            pipeline.addLast("decoder", new PacketDecoder(theirData))
                    .addLast("encoder", new PacketEncoder(ourData));
            if (!this.isMemoryConnection()) {
                pipeline.addLast("zlib_decoder", new Lz4FrameDecoder())
                        .addLast("zlib_encoder", new Lz4FrameEncoder());
            }
        } catch (Exception e) {
    ...
    private static AttributeKey<? extends PacketData<?>> getDataKey(PacketDestination direction) {
        return switch (direction) {
            case CLIENT -> SocketConnection.DATA_TO_CLIENT_KEY;
            case SERVER -> SocketConnection.DATA_TO_SERVER_KEY;
        };
    }
    ...
    /**
     * Get the remote address of the SocketConnection
     *
    ...
    /**
     * Queues the runnable to be executed on the server/client thread.
     *
    ...
    /**
     * Initiates the SocketConnection.
     *
     * @param address     the address to connect to
     * @param port        the port to connect to
     * @param handler     the handler to use for the SocketConnection
    ...
    public void initiate(String address, int port, PacketHandler handler, Packet<?> loginPacket) {
        ...
        this.port = port;
        Connection.LOGGER.info("Initiated SocketConnection to {}:{}", this.address, this.port);
    ...
    /**
     * Runs the consumer once the SocketConnection is connected.
     *
    ...
    /**
     * Checks if the SocketConnection is a memory SocketConnection.
     *
     * @return true if the SocketConnection is a memory SocketConnection, false otherwise
     */
    @Override
    ...
    /**
     * Checks if the SocketConnection is currently connecting.
     *
     * @return true if the SocketConnection is currently connecting, false otherwise
     */
    @Override
    ...
    public void handleDisconnect() {
        if (!this.disconnected) {
            ...
            if (message == null) message = "SocketConnection lost";
    ...
    public Future<?> closeGroup() {
        ...
        if (this.isMemoryConnection()) {
            group = SocketConnection.LOCAL_WORKER_GROUP.get();
        } else {
            group = SocketConnection.NETWORK_WORKER_GROUP.get();
        }
