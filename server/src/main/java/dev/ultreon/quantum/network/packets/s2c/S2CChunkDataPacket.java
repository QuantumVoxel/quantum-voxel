...
/**
 * Represents a packet sent from the server to the client containing data about a specific chunk.
 * This packet provides information regarding the chunk's position, block states, biome data, light map,
 * and block entities within the chunk.
 *
...
public final class S2CChunkDataPacket implements Packet<InGameClientPacketHandler> {
    ...
    public S2CChunkDataPacket(ChunkVec pos, ChunkBuildInfo info, byte[] lightMap, Storage<@NotNull BlockState> storage, @NotNull Storage<@NotNull RegistryKey<Biome>> biomeStorage, IntList blockEntityPositions, IntList blockEntities) {
        this.pos = pos;
        this.info = info;
    ...
    public S2CChunkDataPacket(ChunkVec pos, ChunkBuildInfo info, byte[] lightMap, Storage<@NotNull BlockState> storage, @NotNull Storage<@NotNull RegistryKey<Biome>> biomeStorage, Collection<BlockEntity> blockEntities) {
        this(pos, info, lightMap, storage, biomeStorage, new IntArrayList(), new IntArrayList());

        // Fill block entity positions and block entities
    ...
    public static S2CChunkDataPacket read(PacketIO buffer) {
        ...
        // Read the light map
        var byteLength = buffer.readVarInt();
        var lightMap = new byte[byteLength];
        buffer.readBytes(lightMap);

        // Read block storage and biome storage
        ...
        // Read block entity positions and block entities
        ...
        return new S2CChunkDataPacket(pos, info, lightMap, storage, biomeStorage, blockEntityPositions, blockEntities);
    }
    ...
    public void toBytes(PacketIO buffer) {
        ...
        byte[] save = this.lightMap;
        buffer.writeVarInt(save.length);
        buffer.writeBytes(save);
    ...
    public void handle(PacketContext ctx, InGameClientPacketHandler handler) {
        ...
        handler.onChunkData(this.pos, this.info, this.lightMap, this.storage, this.biomeStorage, blockEntitiesByLocation.get());
    }
