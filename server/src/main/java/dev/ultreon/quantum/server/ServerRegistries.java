...
public class ServerRegistries implements RegistryHandle, Iterable<Registry<?>> {
    ...
    public @NotNull Iterator<Registry<?>> iterator() {
        List<Registry<?>> stream = new ArrayList<>();
        for (Registry<?> registry : registries.values().toArray()) {
