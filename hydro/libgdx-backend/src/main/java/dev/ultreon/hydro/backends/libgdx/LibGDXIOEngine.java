...
public class LibGDXIOEngine implements IOEngine {
    private final Application app;
    private String windowTitle = "Hydro";

    public LibGDXIOEngine(Application app) {
        this.app = app;
    }
    ...
    public PathHandle asset(String path) {
        return new GdxPathHandle(path, Files.FileType.Internal);
    }

    @Override
    public PathHandle data(String path) {
        return new GdxPathHandle(path, Files.FileType.Local).resolve("data");
    }

    @Override
    public PathHandle cache(String path) {
        return new GdxPathHandle(path, Files.FileType.Local).resolve("cache");
    }

    @Override
    public PathHandle config(String path) {
        return new GdxPathHandle(path, Files.FileType.Local).resolve("config");
    }

    @Override
    public PathHandle external(String path) {
        return new GdxPathHandle(path, Files.FileType.External);
    }

    @Override
    public PathHandle absolute(String path) {
        return new GdxPathHandle(path, Files.FileType.Absolute);
    }
