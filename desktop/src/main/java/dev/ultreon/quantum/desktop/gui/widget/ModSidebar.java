package dev.ultreon.quantum.desktop.gui.widget;
...
public class ModSidebar extends ScrollableContainer {
    ...
    public ModSidebar(UIContainer<?> parent) {
        super(200, parent != null ? parent.getSize().height : 0);
        pos.x = -size.width;
        pos.y = 0;
        ...
        descriptionLabel.setSize(300f, parent != null ? parent.getSize().height : 0f);
    ...
    public void openView(ModContainer mod) {
        ...
        String contacts = mod.getMetadata().getContact().asMap().entrySet()
                ...
                .map(entry -> {
                    ...
                    switch (key) {
                        case "website":
                            return "{TRIGGER=URL|" + value + "}[lighter blue]Website{ENDTRIGGER}[lightest grey]";
                        case "homepage":
                            return "{TRIGGER=URL|" + value + "}[lighter blue]Homepage{ENDTRIGGER}[lightest grey]";
                        case "github":
                            return "{TRIGGER=URL|" + value + "}[lighter blue]GitHub{ENDTRIGGER}[lightest grey]";
                        case "gitlab":
                            return "{TRIGGER=URL|" + value + "}[lighter blue]GitLab{ENDTRIGGER}[lightest grey]";
                        case "sources":
                            return "{TRIGGER=URL|" + value + "}[lighter blue]Sources{ENDTRIGGER}[lightest grey]";
                        case "bitbucket":
                            return "{TRIGGER=URL|" + value + "}[lighter blue]BitBucket{ENDTRIGGER}[lightest grey]";
                        case "youtube":
                            return "{TRIGGER=URL|" + value + "}[lighter blue]YouTube{ENDTRIGGER}[lightest grey]";
                        case "twitch":
                            return "{TRIGGER=URL|" + value + "}[lighter blue]Twitch{ENDTRIGGER}[lightest grey]";
                        case "twitter":
                            return "{TRIGGER=URL|" + value + "}[lighter blue]Twitter{ENDTRIGGER}[lightest grey]";
                        case "discord":
                            return "{TRIGGER=URL|" + value + "}[lighter blue]Discord{ENDTRIGGER}[lightest grey]";
                        case "reddit":
                            return "{TRIGGER=URL|" + value + "}[lighter blue]Reddit{ENDTRIGGER}[lightest grey]";
                        case "curseforge":
                            return "{TRIGGER=URL|" + value + "}[lighter blue]CurseForge{ENDTRIGGER}[lightest grey]";
                        case "email":
                            return "{TRIGGER=URL|mailto:" + value + "}[lighter blue]Email{ENDTRIGGER}[lightest grey]";
                        case "issues":
                            return "{TRIGGER=URL|" + value + "}[lighter blue]Issues{ENDTRIGGER}[lightest grey]";
                        case "irc":
                            return "{TRIGGER=URL|" + value + "}[lighter blue]IRC{ENDTRIGGER}[lightest grey]";
                        default:
                            return "(" + key + ")";
                    }
                })
    ...
    public void renderChild(@NotNull Renderer renderer, float deltaTime, Widget widget) {
        widget.getPos().x = pos.x + 10;
        widget.getPos().y = (int) (pos.y - scrollY + widget.getPreferredY());
    ...
    protected void renderBackground(Renderer renderer, float deltaTime) {
        ...
        size.height = getParent() != null ? getParent().size.height : 0;
        pos.y = 0;
        ...
        if (size.width > 200f) {
            renderer.draw9Slice(
                    ...
                    pos.y - 7,
                    size.width - 200 + 7,
                    (getParent() != null ? getParent().size.height : 0) + 14,
                    0, 0,
                    21,
                    21,
                    7,
