...
public class TcpNetworker extends Listener implements Networker {
    ...
    public TcpNetworker(QuantumServer server, @Nullable InetAddress host, int port) throws IOException {
        ...
        this.kryoServer.getKryo().setDefaultSerializer(new PacketIOSerializerFactory(server.getRegistries()));
    ...
    public void disconnected(Connection connection) {
        ...
        ServerTcpConnection remove = this.connections.remove(connection);
        try {
            remove.close();
        } catch (IOException e) {
            CommonConstants.LOGGER.error("Failed to close connection!", e);
        }
    }
    ...
    public void close() throws IOException {
        for (ServerTcpConnection connection : this.connections.values()) {
            connection.disconnect(0, "Server shutting down");
        }
    ...
    public List<? extends IConnection<ServerPacketHandler, ClientPacketHandler>> getConnections() {
        return this.connections.values().stream().map((c) -> (IConnection<ServerPacketHandler, ClientPacketHandler>) c).collect(Collectors.toUnmodifiableList());
    }
